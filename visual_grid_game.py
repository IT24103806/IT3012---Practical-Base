        ...
        for x in range(self.width):
            for y in range(self.height):
                ...
                if position in self.toxic_traps:
                    facts.add('HasDust')
                if not any(tuple(opponent) == position for opponent in self.opponents):
                    facts.add('BloodseekerMissing')
                tile_facts[position] = facts
...
class GridGameGUI:
    ...
    def __init__(self, root, width=10, height=10, num_food=12, num_opponents=2, walls=None):
        ...
        self.cell_size = max(
            20,
            min(max_canvas_dim // self.env.width, max_canvas_dim // self.env.height)
        )
